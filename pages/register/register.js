  ...
  bindGetUserInfo: function(e){
    if (e.detail.userInfo) {
      //用户按了允许授权按钮
        ...
        if (this.data.account == '') {
        ...
        } else if (this.data.password != this.data.re_password) {
          wx.showToast({
          ...
          db.collection('user').where({ //查询接口
              account: that.data.account //传参，输入的用户名
            })
        .get({
              success: function(res) {
                if (res.data.length == 0) { //判断用户名是否被注册过，等于空说明没被查询到，就是没有注册过，
                  db.collection('user').where({ //验证手机号是否被注册过
                    ...
                    .get({
                      success: function(res) {
                        if (res.data.length == 0) { //判断手机号是否被注册过，等于空说明没被查询到，就是没有注册过，
                          ...
                          db.collection('user').add({ //验证完成之后，添加的接口
                            data: {
